...
**UI W1~W3 — 위험 지도 공간·카테고리 열 구분·시각 정체성 (2026-07-11 사용자 승인).**

- **W1 유동 폭**: `.app-main` 1200px 상한 제거 — 브라우저 크기를 그대로 따른다.
  긴 산문(ask 답변 등)만 `ch` 상한으로 가독성 유지.
- **W2 열 카테고리 구분**: heatmap thead 2단(카테고리 그룹 행) + 열 배경 틴트.
  라벨은 `VALUE_LABELS`에 `ip_category` 도메인 추가(커버리지 테스트 포함).
- **W3 시각 정체성(관제 코크핏)**: 셀=배경 채움 타일(심볼 병행, 색약 대응),
  팔레트 교체(딥 인디고 악센트·다크 헤더), 타이포 위계, 카드 폴리시.
  위험색 의미(빨/노/초) 불변, 수치 점수 금지 등 기존 불변 원칙 유지.
- 단계별 commit. 검증: frontend build/test/lint + axe smoke + (W2) backend
  glossary 테스트. 완료 후 Chrome 실화면 스크린샷으로 톤 확인 받기.
