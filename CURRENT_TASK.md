...
> - **구현 완료 (2026-07-20) + 개선 2회 (2026-07-21, 사용자 제안)**:
>   ① 드롭다운 → 주차 순 칩 → **연간 주차 캘린더**(W1~W52 SVG 축·분기
>   눈금·현재 주차 선·게이트 상시 라벨) ② **전 마일스톤 선택 가능**(기준
>   미정의 = 정보+정직 패널). backend 337 · frontend 53 green + 리허설
>   DB(8155) 실서버 확인 완료. 커밋 `33c1994`→`05e7a44`→`b61be7e`.
>   **사용자 "이거다" 판정 대기 — 판정 후 G2 이후 계속 여부 결정 (정지 상태).**
>   G1 판정 입력으로 리허설 체크리스트 트랙 C 3문항 사용. 보류 메모:
>   주차>52 다년 과제 축 정의(사내 주차 체계 확정 시), 3과제 스윔레인
>   단일 축(G6 간트와 결합 후보).
>
