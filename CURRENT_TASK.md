...
**UI G1~G3 — 변경 영향 화면 재설계: 영향 전파 지도 (2026-07-11 사용자 승인).**

- **G2 문장형 질의 빌더 + 요약 스트립**: 셀렉트 나열 → "[IP]의 [knob]을 [모드]에서
  바꾸면?" 문장 폼. 결과 상단 계기판 요약(시나리오/KPI/연쇄 IP/역할/사례 건수).
- **G1 영향 전파 지도**: 계층형 흐름 그래프(순수 SVG, 신규 의존성 없음) —
  연쇄 IP ↔ 분석 대상 → 영향 시나리오 → KPI. 간선=근거 규칙(색 구분),
  노드 클릭 → 우측 근거 패널(위험 지도와 동일 문법, 스플리터 공용 컴포넌트로 추출).
- **G3 재편**: 시나리오/KPI/연쇄 카드 → 그래프+패널로 흡수, 하단은 체크리스트+
  유사 사례. `?node=` URL 상태. 접근성(노드=버튼, axe 게이트에 화면 추가).
- 이후에도 개선 가능 항목 계속 진행(사용자 위임) — 단계별 commit.
- 직전 완료: **UI W1~W3+스플리터** — 유동 폭/열 카테고리 구분/코크핏 정체성
  (CHANGELOG 참조).
