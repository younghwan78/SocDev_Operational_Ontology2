...
**what-if 워크벤치 — 위험 지도 가정 실험 모드 (2026-07-17 승인, 설계 18)**

> 사용자 지적: 현 UI는 이슈 상세 1클릭 2종뿐 — 엔진(가정 4종·복합 10개)의 탐색형
> 사용이 불가. "다양한 선택지를 미리 보여주고 결정 scope을 넓히는" UX로 확장한다.
> 기준: `internal_docs/design/18_whatif_workbench.md`.
>
> - W1 백엔드: `GET /what-if/candidates` — 기존 신호에서 결정론으로 가정 후보 도출
>   (검증 없는 종결/미해결 고심각/목표 주차 보유/위험 일정 이벤트). 판정·저장 계약 무변경.
> - W2 프론트: 위험 지도 가정 실험 모드 — URL `whatif`=가정 세트(공유 가능),
>   후보 패널+바스켓(≤10)+신규 이슈 주입 폼, 지도 셀 기준→투영 오버레이, 이슈 신호 델타.
>
> 원칙: 결정론 / 쓰기 경로 신설 없음 / 수치 점수 금지 / 가정 confidence ≤ medium /
> 한국어 1급 / 후보는 제안일 뿐 — 시스템이 결정하지 않는다.
>
> **다음 후보 (착수는 사용자 승인)**:
