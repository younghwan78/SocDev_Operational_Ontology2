...
**UI E1~E6 — 탐색 화면 6종 폴리싱 (2026-07-12 사용자 승인, goal 진행).**

> 포트폴리오/시나리오/리뷰 센터/근거 탐색/출처 지도/데이터 반입을 화면별로
> 파악→개선→commit. 확립된 문법 재사용: 상황판 숫자 카드(클릭=필터), 세그먼트
> 막대, URL=상태, 한국어 값 라벨, 화면 간 링크, 접기/펼치기. 불변 원칙 유지.
