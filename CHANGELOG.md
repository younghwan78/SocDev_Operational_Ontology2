...
## 설계 26 G1 — A0 판정 배너 + 게이트 콘솔 홈 (2026-07-20)

> 설계: `internal_docs/design/26_gate_anchored_ux.md` §5 G1 (§8.1 결정 기록:
> ① 홈 정박 전환 승인 ② kind 2종 확장 승인 — 구현은 G2 ③ G1 선행 후 판단).
> 재작성이 아니라 재정박 — 설계 23 게이트 판정·W2 연결률·반입 배치를 A0
> 배너로 조립. 저장 계약 변경 없음, GO/NO-GO 어휘 금지(판정은 조언).

- **`GateConsoleService`** (`backend/services/gate_console.py`) +
  `GET /gate-console`: 과제별 게이트 **자동 선택**(기준 주차=데이터 최신
  활동 주차 이후 최근접, exit_criteria 보유 — 전부 과거면 최신 게이트를
  이유와 함께, 없으면 "게이트 미지정" 정직 표기) + **판정 한 줄**(충족/
  미충족/판정 불가 + **지배 요인** 결정론 룰: kind 우선순위 이슈 수 최다 →
  근거 누락 → 검증 없는 종결, 위반 수 내림차순) + **신뢰도 줄**(프로젝트
  이슈 연결률(W2 LINK_FIELDS 재사용) + 최근 완료 배치 시각 — "판정이 못
  보는 것"을 배너가 말한다). 드롭다운 전환용 전 게이트 판정 동봉(재요청 없음).
- **홈 전환** (`frontend`): `/` = 게이트 콘솔(`GateConsolePage`), 위험
  지도는 `/risk-map`으로 이동 — 딥링크 3곳(변경 영향 what-if 브리지·RCA
  what-if·시나리오 개요 지도 링크)과 데모 스토리 장면1 경로 동반 갱신.
  now-what: 지배 요인 드릴(이슈 분석/근거 탐색) + 위험 지도 + 리뷰 센터.
  기준별 판정 상세는 설계 23 `MilestoneGates` 재사용(접힘).
- **리허설 UX 체크리스트 트랙 C 3문항** 추가 (3초 판정/지배 요인/신뢰도 —
  Δ지난 게이트 문항은 G2에서).
- 검증: backend 336 · frontend 51 green, ruff/mypy/lint/validate-data 통과.
  리허설 DB 실서버 — U W12 게이트(미해결 이슈 1건 지배), W 게이트 2종
  드롭다운(검증 없는 종결 15건 지배), V 게이트 미지정, 연결률 46%/23% 표시.
- **게이트 타임라인 칩 (2026-07-21, 사용자 제안)**: 드롭다운 → **주차 순 전
  마일스톤 칩 스트립** 교체. `GateTimelineEntry`(additive) — ●=exit 기준
  보유(판정 색 점, 클릭=배너 로컬 전환), ○=기준 미정의 유령 칩(정직 표기 —
  게이트 정의 캠페인 유도), "▸ 현재 W{기준 주차}" 마커(전부 과거면 맨 뒤).
  V 과제가 "미지정 한 줄" 대신 전체 일정(6개)을 보여주게 됨. A0 3초 판정은
  유지 — 타임라인은 선택기, 배너는 그대로. backend 337 · frontend 52 green,
  리허설 DB 실서버 확인(U 10·V 6·W 6 마일스톤, W21↔W24 칩 전환).
- **연간 주차 캘린더 전환 (2026-07-21, 사용자 제안 2차)**: 칩 나열 →
  **W1~W52 고정 축 inline SVG 캘린더** (KPI 차트와 동일 태도, 라이브러리
  없음). 분기 눈금(Q1~Q4)+주차 눈금, 마일스톤을 주차 위치에 마커로 배치
  (근접 마커·라벨 겹침 회피 stagger), "현재 주차" 점선, 게이트 마커만 상시
  라벨(훑어보기 복원 — 과제명 접두 제거+축약). **전 마일스톤 선택 가능**:
  게이트=판정 배너, 기준 미정의=마일스톤 정보(제목·주차·분기·설명) +
  "기준을 정의하면 게이트로 판정된다" 정직 패널 + 신뢰도 줄.
  `GateTimelineEntry`에 description/quarter 추가(additive). 주차 미상
  마일스톤은 축 아래 칩. backend 337 · frontend 53 green, 실서버 확인.
