# 장기 개선 계획 — Stage 13~19 (운영 시스템 완성 로드맵)

> 상태: v1.0 제안 (2026-07-06, Stage 8~12 완료 시점 기준)
...
|---|---|---|---|
| G1 | 정보 탐색 시간 단축 | 위험 지도 홈 + Ask SoC + 데모 TAT 로그 | 실데이터 위에서 주간 리뷰 준비 30%+ 단축을 **측정으로 입증** |
...
| L4 | 검색이 키워드 토큰 매칭 — 한국어 시맨틱 없음, 단일 홉 | Ask SoC 재현율 한계 (G1·G5 제약) | Stage 17 |
...
| L7 | TAT 로그가 localStorage 한정 — 서버 수집 없음 | 효과 측정을 집계 못 함 | Stage 16 |
| L8 | 이벤트↔IP 명시 링크 부재 (휴리스틱 귀속) | 위험 셀/유사 사례 정밀도 한계 | Stage 15(계약 확장) |
| L9 | G4 지표(close evidence 연결률)가 화면에 상시 노출되지 않음 | 품질 신호 미가시화 | Stage 16 |
...
| L12 | 반응형/접근성 미검증 (데스크톱 1500px 기준) | 회의실 대화면/노트북 외 환경 | Stage 16 |
...
```mermaid
flowchart LR
    S13[Stage 13<br/>워크숍 검증+교정 루프] --> S14[Stage 14<br/>운영 기반 경화<br/>CI·인증·배포·캐시]
    S13 --> S15[Stage 15<br/>실데이터 반입 확대<br/>+계약 정밀화]
    S14 --> S16[Stage 16<br/>운영 파일럿+효과 지표]
    S15 --> S16
    S15 --> S17[Stage 17<br/>시맨틱 검색+Ask 고도화]
    S16 --> S18[Stage 18<br/>JIRA/Confluence 커넥터]
    S17 --> S18
    S18 --> S19[Stage 19<br/>확장: 타 도메인·예측 위험]
```

- 순서 원칙 (원점 문서): "ingestion 자동화보다 연결 모델 검증이 먼저" → 13이 최우선.
- 커넥터(18)는 보안 승인이 선행 조건이므로 뒤에 배치하되, 승인 절차는 13 시점에 착수 요청.
...
2. **워크숍 실행** (사람): `internal_docs/validation/` 자료로 60분 세션, 참가 3~6명.
3. **교정 반영** (개발 항목): 판정 "아니다/조건부" 항목을 백로그화 —
...
4. `internal_docs/validation/03_workshop_results_<날짜>.md` — 판정 결과/교정 결정 기록.
...
- [ ] 워크숍 1회 실행 → TAT 기준표 Q1~Q5 실측치 기입 완료
...
## 6. Stage 16 — 운영 파일럿 + 효과 지표 상시화 [규모: M]
...
## 7. Stage 17 — 한국어 시맨틱 검색 + Ask SoC 고도화 [규모: L]
...
## 8. Stage 18 — JIRA/Confluence read-only 커넥터 [규모: L, 보안 승인 선행]

**목적**: 주 1회 CSV 수작업을 자동 동기화로 대체 (G1). **원점 원칙상 13~17로 연결 모델이
검증된 뒤에만 착수.**
...
## 9. Stage 19+ — 확장 실험 (탐색적, 각각 사용자 승인 필요)
