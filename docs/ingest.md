...
1. **매핑 선택** — 무엇을 반입하나: 개발 이슈 / 검증 테스트 / 개발 이벤트 /
   근거 카탈로그 / 측정 근거 / 리뷰 결정 / 액션 아이템 등. 필수 열이 함께 표시되고,
   **"열 스펙 (허용 값·형식)"을 펼치면** 열마다 필수 여부·형식(정수/목록/예·아니오)·
   허용되는 상태/유형 코드와 한국어 라벨이 표로 나옵니다 — 채우기 전에 먼저 보세요.
2. **템플릿 CSV 내려받기** — 열 이름(한국어)이 계약입니다. 엑셀로 채우세요.
   리스트 값은 세미콜론(`;`)으로 구분합니다 (예: `uhd60_recording_eis_on;apv_recording`).
3. **검사만 실행 (권장)** — 반입 전에 같은 파일로 **검사만 실행**을 누르면
   신규/갱신/거부 결과와 품질 리포트를 똑같이 계산하되 **아무것도 저장하지 않습니다**.
   대량 갱신은 롤백해도 이전 내용이 복원되지 않으므로, 결과를 확인한 뒤
   **반입 실행**으로 확정하는 2단계를 습관으로 삼으세요.
   **작성자** 칸에 이름을 넣으면 배치 기록에 남습니다 (누가 반입했는지 추적).
4. **업로드** — 결과가 4개 숫자로 나옵니다:
   - **신규** — 처음 들어온 행
   - **갱신** — 같은 ID의 내용이 바뀌어 교체됨
   - **변동 없음** — 같은 ID·같은 내용이라 건너뜀 (**같은 파일을 다시 올려도 안전**)
   - **거부** — 필수 열 누락 등. 사유가 행 번호와 함께 한국어로 표시됩니다.
5. **품질 확인** — 반입 성공 ≠ 활용 가능. 세 가지를 봅니다:
