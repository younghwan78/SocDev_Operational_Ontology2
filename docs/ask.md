...
![자주 묻는 질문·최근 질문](assets/ask-faq.png)

- **자주 묻는 질문 / 최근 질문** — 질문하기 전 대기 화면에 표시됩니다. 모든 질의는
  기록으로 남아 같은 질문의 횟수가 집계되고, 좋은 질문이 자연스럽게 예제 목록이 됩니다.
  클릭하면 그대로 재질의됩니다.
- **관련 객체가 먼저 뜹니다** — 질문을 제출하면 검색(결정론)이 즉시 우측에 카드를
  보여주고, 답변은 그 사이 생성됩니다. 카드가 엉뚱하면 답변을 기다릴 필요 없이
  질문을 고치면 됩니다. "매치되지 않은 용어"가 표시되면 그 단어는 검색이 못 찾은 것입니다.
- **본문 각주 ①②③** — 답변 문장 끝의 번호가 곧 인용입니다. 클릭하면 우측의 해당
  근거 카드가 강조됩니다. 어떤 문장이 어떤 근거에 기대는지 문장 단위로 추적됩니다.
- **캐시 응답** — 같은 질문을 같은 데이터 상태에서 다시 하면 저장된 답변을 즉시
  돌려줍니다(0.x초). 배지에 원 질의 시각이 표시되며, 데이터가 바뀌면 자동으로 새로 답합니다.
