...
1. **필터** — 프로젝트(U/V/W), 검증 상태(검증됨/검증 미완/검증 테스트 없음).
2. **이슈 목록** (좌측) — 각 이슈에 검증 상태 뱃지가 붙습니다.
   **왼쪽에 빨간 줄이 있는 이슈 = 종결됐지만 검증되지 않은 이슈**로, 목록 맨 위에 옵니다.
3. **RCA 체인** (우측) — 이슈를 클릭하면 7단 세로 흐름이 표시됩니다.
